...
const Itsoapp = () => {
    ...
    const fetchBookedTimeSlots = async () => {
        try {
            const response = await fetch(`http://localhost/asbackend/availtimeitso.php?date=${date}`);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch('http://localhost/asbackend/itsoapp.php', {
                method: 'POST',
                headers: {
            ...
            if (data.status === 'success') {
                ...
                navigate('/itsoconpage', { state: { date, timeSlot } });
            }
        } catch (error) {
